...
### 13.2 生产环境关闭 `diagnose`

```python
from loguru import logger

logger.remove()
logger.add("app.log", diagnose=False, backtrace=True, encoding="utf-8")
```

原因很简单: 调试信息越丰富，泄露敏感数据的风险越高。

### 13.3 多进程场景使用 `enqueue=True`

官方文档指出，`enqueue=True` 会让日志先进入一个多进程安全的队列，这对多进程写文件很重要。

```python
from loguru import logger

logger.remove()
logger.add("multiprocess.log", enqueue=True, encoding="utf-8")
```
...
```python
logger.info("用户1001在上海登录，订单ORD-1支付成功")
```

不如写:

```python
logger.bind(user_id=1001, city="Shanghai", order_id="ORD-1").info("支付成功")
```
...
如果你的格式里引用了 `{extra[request_id]}` 这类字段，最好先配置默认值:
...
这样即使某些地方没有绑定上下文，也不会让日志格式变得混乱。

### 13.7 类库开发者考虑 `disable()`

官方文档特别提醒: 如果你在开发一个库，而不是一个最终应用，应该考虑禁用默认日志输出，避免打扰使用者。
